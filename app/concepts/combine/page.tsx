...
import { BookOpen, Code2, Lightbulb, GitCompare, Trophy, ChevronLeft } from 'lucide-react';
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter';
import { vscDarkPlus } from 'react-syntax-highlighter/dist/esm/styles/prism';

export default function CombinePage() {
  // Only show Combine level (levelNumber 0)
  const combineLevel = learningLevels.find(level => level.levelNumber === 0) || learningLevels[0];
  const [selectedWeek, setSelectedWeek] = useState(combineLevel.weeks[0]);
  const [selectedConcept, setSelectedConcept] = useState(combineLevel.weeks[0].concepts[0]);
